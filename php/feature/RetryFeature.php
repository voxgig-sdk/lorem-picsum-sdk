...
class LoremPicsumRetryFeature extends LoremPicsumBaseFeature
{
    ...
    public function init(LoremPicsumContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LoremPicsumContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LoremPicsumContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LoremPicsumContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
