...
class GetRandomImageEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LoremPicsumSDK::test(null, null);
        $ent = $testsdk->GetRandomImage(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = get_random_image_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "get_random_image." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LOREMPICSUM_TEST_GET_RANDOM_IMAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $get_random_image_ref01_data = null;
        if (count($get_random_image_ref01_data_raw) > 0) {
        ...
        [$get_random_image_ref01_data_dt0_loaded, $err] = $get_random_image_ref01_ent->load($get_random_image_ref01_match_dt0, null);
        $this->assertNull($err);
...
function get_random_image_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/get_random_image/GetRandomImageTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["get_random_image01", "get_random_image02", "get_random_image03", "width01"] as $k) {
    ...
    $entid_env_raw = getenv("LOREMPICSUM_TEST_GET_RANDOM_IMAGE_ENTID");
