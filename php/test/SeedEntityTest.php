...
class SeedEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LoremPicsumSDK::test(null, null);
        $ent = $testsdk->Seed(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = seed_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "seed." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LOREM_PICSUM_TEST_SEED_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $seed_ref01_data = null;
        if (count($seed_ref01_data_raw) > 0) {
...
function seed_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/seed/SeedTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["seed01", "seed02", "seed03", "width01"] as $k) {
    ...
    $entid_env_raw = getenv("LOREM_PICSUM_TEST_SEED_ENTID");
