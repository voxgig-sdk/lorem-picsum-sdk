...
class SeedInfoEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LoremPicsumSDK::test(null, null);
        $ent = $testsdk->SeedInfo(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = seed_info_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "seed_info." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $seed_info_ref01_data = null;
        if (count($seed_info_ref01_data_raw) > 0) {
            $seed_info_ref01_data = Helpers::to_map($seed_info_ref01_data_raw[0][1]);
        }

        // LOAD
        $seed_info_ref01_ent = $client->SeedInfo(null);
...
function seed_info_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/seed_info/SeedInfoTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["seed_info01", "seed_info02", "seed_info03"] as $k) {
