...
class IdnEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LoremPicsumSDK::test(null, null);
        $ent = $testsdk->Idn(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = idn_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "idn." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LOREMPICSUM_TEST_IDN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function idn_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/idn/IdnTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["idn01", "idn02", "idn03", "width01"] as $k) {
    ...
    $entid_env_raw = getenv("LOREMPICSUM_TEST_IDN_ENTID");
