...
describe('SeedDirect', async () => {
  ...
  test('direct-load-seed', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-seed', setup.live)) return
    if (skipIfMissingIds(t, setup, ["height01","seed01","width01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LOREM_PICSUM_TEST_SEED_ENTID']
