...
describe('IdnDirect', async () => {
  ...
  test('direct-load-idn', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-idn', setup.live)) return
    if (skipIfMissingIds(t, setup, ["height01","id01","width01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LOREM_PICSUM_TEST_IDN_ENTID']
