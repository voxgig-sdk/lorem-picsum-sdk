...
describe('HeightwebpDirect', async () => {
  ...
  test('direct-load-heightwebp', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-heightwebp', setup.live)) return
    if (skipIfMissingIds(t, setup, ["width01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LOREM_PICSUM_TEST_HEIGHTWEBP_ENTID']
