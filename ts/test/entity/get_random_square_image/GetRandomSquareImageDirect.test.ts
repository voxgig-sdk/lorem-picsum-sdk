...
describe('GetRandomSquareImageDirect', async () => {
  ...
  test('direct-load-get_random_square_image', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_random_square_image', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LOREM_PICSUM_TEST_GET_RANDOM_SQUARE_IMAGE_ENTID']
