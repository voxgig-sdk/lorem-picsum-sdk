...
module LoremPicsumUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LoremPicsumResponse.new({})
    ...
    if fetch_err
      ...
      response = LoremPicsumResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LoremPicsumResponse.new(fetched)
    else
