...
class HeightwebpDirectTest < Minitest::Test
  def test_direct_load_heightwebp
    setup = heightwebp_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-heightwebp", setup[:live] ? "live" : "unit")
    if _should_skip
...
def heightwebp_direct_setup(mockres)
