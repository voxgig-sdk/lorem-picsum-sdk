...
class HeightDirectTest < Minitest::Test
  def test_direct_load_height
    setup = height_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-height", setup[:live] ? "live" : "unit")
    if _should_skip
...
def height_direct_setup(mockres)
