...
class IdnEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = idn_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "idn." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LOREMPICSUM_TEST_IDN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    idn_ref01_data = nil
    if idn_ref01_data_raw.length > 0
    ...
    idn_ref01_match_dt0 = {}
    idn_ref01_data_dt0_loaded, err = idn_ref01_ent.load(idn_ref01_match_dt0, nil)
    assert_nil err
...
def idn_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "idn", "IdnTestData.json")
  ...
  entid_env_raw = ENV["LOREMPICSUM_TEST_IDN_ENTID"]
