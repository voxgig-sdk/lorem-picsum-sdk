...
class HeightEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = height_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "height." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    height_ref01_data = nil
    if height_ref01_data_raw.length > 0
...
def height_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "height", "HeightTestData.json")
