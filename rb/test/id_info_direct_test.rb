...
class IdInfoDirectTest < Minitest::Test
  def test_direct_load_id_info
    setup = id_info_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-id_info", setup[:live] ? "live" : "unit")
    if _should_skip
...
def id_info_direct_setup(mockres)
