...
The Ruby SDK for the LoremPicsum API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.GetRandomImage(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.LoremPicsum(nil).load(
  { "id" => "test01" }, nil
)
