...
func TestHeightEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		heightRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.height")))
...
func heightBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LOREM_PICSUM_TEST_HEIGHT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LOREM_PICSUM_TEST_HEIGHT_ENTID"])
