...
import (
	...
	omni "github.com/voxgig-sdk/lorem-picsum-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LoremPicsumError); is {
				out := map[string]any{
					"name":    "LoremPicsumError",
...
func omniCtx(arg any, client *sdk.LoremPicsumSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LoremPicsumSDK); is {
			client = live
