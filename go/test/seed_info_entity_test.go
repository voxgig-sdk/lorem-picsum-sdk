...
func TestSeedInfoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		seedInfoRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.seed_info")))
		...
		seedInfoRef01DataDt0LoadResult := core.ToMapAny(entityData(seedInfoRef01DataDt0Loaded))
...
func seed_infoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LOREM_PICSUM_TEST_SEED_INFO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LOREM_PICSUM_TEST_SEED_INFO_ENTID"])
