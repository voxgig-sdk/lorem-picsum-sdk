...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("LOREM_PICSUM_TEST_LIVE") == "TRUE" ||
		os.Getenv("LOREM_PICSUM_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("LOREM_PICSUM_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.LoremPicsumSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.LoremPicsumError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.LoremPicsumSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.LoremPicsumError{Msg: msg, Code: code}
}
