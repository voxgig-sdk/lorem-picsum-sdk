...
func TestIdnEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LOREMPICSUM_TEST_IDN_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func idnBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LOREMPICSUM_TEST_IDN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LOREMPICSUM_TEST_IDN_ENTID"])
