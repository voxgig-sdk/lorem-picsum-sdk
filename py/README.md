...
The Python SDK for the LoremPicsum API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetRandomImage(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.LoremPicsum(None).load(
    {"id": "test01"}, None
)
