...
class SeedEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SeedEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Seed:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Seed().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
