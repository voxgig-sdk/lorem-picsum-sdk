...
class TestHeightDirect:

    def test_should_direct_load_height(self):
        setup = _height_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-height", "live" if setup["live"] else "unit")
        if _skip:
...
def _height_direct_setup(mockres):
