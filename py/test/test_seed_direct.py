...
class TestSeedDirect:

    def test_should_direct_load_seed(self):
        setup = _seed_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-seed", "live" if setup["live"] else "unit")
        if _skip:
...
def _seed_direct_setup(mockres):
