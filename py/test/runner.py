...
class LoremPicsumTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LoremPicsumTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LoremPicsumTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LoremPicsumTestRunner.getenv("LOREM_PICSUM_TEST_LIVE")
        override = LoremPicsumTestRunner.getenv("LOREM_PICSUM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LoremPicsumTestRunner.getenv(key)
        ...
        explain = LoremPicsumTestRunner.getenv("LOREM_PICSUM_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if LoremPicsumTestRunner._test_control is not None:
            return LoremPicsumTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LoremPicsumTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LoremPicsumTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LoremPicsumTestRunner.load_test_control()
...
def load_env_local():
    LoremPicsumTestRunner.load_env_local()


def env_override(m):
    return LoremPicsumTestRunner.env_override(m)


def entity_data(v):
    return LoremPicsumTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LoremPicsumTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LoremPicsumTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LoremPicsumTestRunner.load_test_control()


def live_client_options():
    return LoremPicsumTestRunner.live_client_options()


def live_delay_ms():
    return LoremPicsumTestRunner.live_delay_ms()
