...
class TestSeedInfoDirect:

    def test_should_direct_load_seed_info(self):
        setup = _seed_info_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-seed_info", "live" if setup["live"] else "unit")
        if _skip:
...
def _seed_info_direct_setup(mockres):
