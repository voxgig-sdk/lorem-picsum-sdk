...
class TestIdnDirect:
    ...
    def test_should_direct_load_idn(self):
        setup = _idn_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-idn", "live" if setup["live"] else "unit")
        if _skip:
