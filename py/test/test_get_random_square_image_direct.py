...
class TestGetRandomSquareImageDirect:

    def test_should_direct_load_get_random_square_image(self):
        setup = _get_random_square_image_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get_random_square_image", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_random_square_image_direct_setup(mockres):
