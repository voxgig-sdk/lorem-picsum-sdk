...
class TestSeedInfoEntity:

    def test_should_create_instance(self):
        testsdk = LoremPicsumSDK.test(None, None)
        ent = testsdk.SeedInfo(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _seed_info_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "seed_info." + _op, "live" if _live else "unit")
            if _skip:
        ...
        seed_info_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(seed_info_ref01_data_dt0_loaded))
...
def _seed_info_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/seed_info/SeedInfoTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "LOREM_PICSUM_TEST_SEED_INFO_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOREM_PICSUM_TEST_SEED_INFO_ENTID"))
