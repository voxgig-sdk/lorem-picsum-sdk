...
class TestIdInfoDirect:

    def test_should_direct_load_id_info(self):
        setup = _id_info_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-id_info", "live" if setup["live"] else "unit")
        if _skip:
...
def _id_info_direct_setup(mockres):
