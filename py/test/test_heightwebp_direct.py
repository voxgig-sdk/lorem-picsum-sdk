...
class TestHeightwebpDirect:

    def test_should_direct_load_heightwebp(self):
        setup = _heightwebp_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-heightwebp", "live" if setup["live"] else "unit")
        if _skip:
...
def _heightwebp_direct_setup(mockres):
