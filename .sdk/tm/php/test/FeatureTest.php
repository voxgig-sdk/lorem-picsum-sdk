...
class FtCtrl extends LoremPicsumControl
{
...
class FtClient
{
    ...
    private LoremPicsumUtility $utility;

    public function __construct(LoremPicsumUtility $utility, array $options)
    {
    ...
    public function get_utility(): LoremPicsumUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LoremPicsumContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LoremPicsumContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LoremPicsumContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LoremPicsumConfig::make_feature($name);
    ...
    private static function buildUrl(LoremPicsumSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LoremPicsumError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LoremPicsumResponse($fetched);
        }
    ...
    private function fail(LoremPicsumContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LoremPicsumContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
