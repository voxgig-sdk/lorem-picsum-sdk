...
class LoremPicsumMakeRequest
{
    public static function call(LoremPicsumContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new LoremPicsumResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new LoremPicsumResponse($fetched);
        } else {
