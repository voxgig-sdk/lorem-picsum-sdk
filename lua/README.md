...
The Lua SDK for the LoremPicsum API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:GetRandomImage()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── lorem-picsum_sdk.lua    -- Main SDK module
...
The main module (`lorem-picsum_sdk`) exports the SDK constructor
