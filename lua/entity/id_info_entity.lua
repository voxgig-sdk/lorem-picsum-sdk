...
local IdInfoEntity = {}
IdInfoEntity.__index = IdInfoEntity


function IdInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IdInfoEntity)
  self._name = "id_info"
  self._client = client
...
function IdInfoEntity:get_name()
  return self._name
end


function IdInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IdInfoEntity.new(self._client, opts)
end
...
function IdInfoEntity:mark_deleted()
  self._deleted = true
end


function IdInfoEntity:deleted()
  return true == self._deleted
end


function IdInfoEntity:data_set(args)
  if args ~= nil then
...
function IdInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IdInfoEntity:match_set(args)
  if args ~= nil then
...
function IdInfoEntity:match_get()
...
function IdInfoEntity:stream(action, args, callopts)
...
function IdInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
