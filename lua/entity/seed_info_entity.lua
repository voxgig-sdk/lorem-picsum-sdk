...
local SeedInfoEntity = {}
SeedInfoEntity.__index = SeedInfoEntity


function SeedInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SeedInfoEntity)
  self._name = "seed_info"
  self._client = client
...
function SeedInfoEntity:get_name()
  return self._name
end


function SeedInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SeedInfoEntity.new(self._client, opts)
end
...
function SeedInfoEntity:mark_deleted()
  self._deleted = true
end


function SeedInfoEntity:deleted()
  return true == self._deleted
end


function SeedInfoEntity:data_set(args)
  if args ~= nil then
...
function SeedInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SeedInfoEntity:match_set(args)
  if args ~= nil then
...
function SeedInfoEntity:match_get()
...
function SeedInfoEntity:stream(action, args, callopts)
...
function SeedInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
