...
local SeedEntity = {}
SeedEntity.__index = SeedEntity


function SeedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SeedEntity)
  self._name = "seed"
  self._client = client
...
function SeedEntity:get_name()
  return self._name
end


function SeedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SeedEntity.new(self._client, opts)
end
...
function SeedEntity:mark_deleted()
  self._deleted = true
end


function SeedEntity:deleted()
  return true == self._deleted
end


function SeedEntity:data_set(args)
  if args ~= nil then
...
function SeedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SeedEntity:match_set(args)
  if args ~= nil then
...
function SeedEntity:stream(action, args, callopts)
...
---@param reqmatch SeedLoadMatch
---@param ctrl? table
---@return Seed
---@return string? err
function SeedEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SeedEntity:_run_op(ctx, post_done)
  local utility = self._utility
