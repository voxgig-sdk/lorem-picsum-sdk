...
local IdnEntity = {}
IdnEntity.__index = IdnEntity


function IdnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IdnEntity)
  self._name = "idn"
  self._client = client
...
function IdnEntity:get_name()
  return self._name
end


function IdnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IdnEntity.new(self._client, opts)
end
...
function IdnEntity:mark_deleted()
  self._deleted = true
end


function IdnEntity:deleted()
  return true == self._deleted
end


function IdnEntity:data_set(args)
  if args ~= nil then
...
function IdnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IdnEntity:match_set(args)
  if args ~= nil then
...
function IdnEntity:stream(action, args, callopts)
...
---@param reqmatch IdnLoadMatch
---@param ctrl? table
---@return Idn
---@return string? err
function IdnEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function IdnEntity:_run_op(ctx, post_done)
  local utility = self._utility
