...
local HeightEntity = {}
HeightEntity.__index = HeightEntity


function HeightEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HeightEntity)
  self._name = "height"
  self._client = client
...
function HeightEntity:get_name()
  return self._name
end


function HeightEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HeightEntity.new(self._client, opts)
end
...
function HeightEntity:mark_deleted()
  self._deleted = true
end


function HeightEntity:deleted()
  return true == self._deleted
end


function HeightEntity:data_set(args)
  if args ~= nil then
...
function HeightEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HeightEntity:match_set(args)
  if args ~= nil then
...
function HeightEntity:match_get()
...
function HeightEntity:stream(action, args, callopts)
...
function HeightEntity:_run_op(ctx, post_done)
  local utility = self._utility
