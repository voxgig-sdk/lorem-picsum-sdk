...
-- by passing it.
function LoremPicsumSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function LoremPicsumSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "LoremPicsumSDK: graphql: " .. msg
    res.graphql = errors
  end
