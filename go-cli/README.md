...
boru-driven command-line client **and** interactive REPL for the LoremPicsum
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lorem-picsum-cli)
make build

# 2. See usage (words, entities, env vars)
./lorem-picsum-cli --help

# 3. Provide credentials once, via the environment
export LOREM_PICSUM_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./lorem-picsum-cli load 1 get_random_image            # {id:1} shorthand
./lorem-picsum-cli load '{id:1}' get_random_image       # explicit match map

# 5. Override the API base URL for a single call
LOREM_PICSUM_BASE=https://api.example.com ./lorem-picsum-cli load 1 get_random_image

# 6. No arguments -> interactive REPL
./lorem-picsum-cli
lorem-picsum> load 1 get_random_image
lorem-picsum> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lorem-picsum-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/lorem-picsum-cli load 1 get_random_image
   ```
...
```sh
./lorem-picsum-cli load 1 get_random_image          # scalar shorthand for {id:1}
./lorem-picsum-cli load '{id:1}' get_random_image     # explicit match map
```
...
./lorem-picsum-cli load 1 get_random_image
```
...
Run with no arguments to open a REPL (prompt `lorem-picsum>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_random_image`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/lorem-picsum-cli`. |
...
(`./lorem-picsum-cli <expr>`) and interactively (the REPL), and expressions compose the
