// Command LoremPicsum-cli is a CLI + REPL for the LoremPicsum SDK,
...
const prompt = "lorem-picsum"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_random_image get_random_square_image height heightwebp id_info idn list seed seed_info"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LOREM_PICSUM_APIKEY (api key), LOREM_PICSUM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LOREM_PICSUM_APIKEY carries the API key and
	// LOREM_PICSUM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LOREM_PICSUM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LOREM_PICSUM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLoremPicsumSDK(opts)
