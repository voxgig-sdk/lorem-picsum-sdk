...
[MCP](https://modelcontextprotocol.io) server exposing the LoremPicsum SDK as
two agent tools — `lorem-picsum_list` and `lorem-picsum_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lorem-picsum-mcp)
make build

# 2. Provide credentials via the environment
export LOREM_PICSUM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user lorem-picsum \
  -- /absolute/path/to/lorem-picsum-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./lorem-picsum-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// lorem-picsum_list: first page of records
{ "entity": "list" }
{ "entity": "list", "query": { } }

// lorem-picsum_load: one record by id
{ "entity": "get_random_image", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lorem-picsum-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user lorem-picsum \
     -- "$PWD"/dist/*/lorem-picsum-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `lorem-picsum_list` and `lorem-picsum_load` tools now appear
   in new sessions. Ask the agent to *"list list using lorem-picsum"*
   and it calls `lorem-picsum_list` with `{"entity":"list"}`.
...
```sh
export LOREM_PICSUM_APIKEY=sk_live_xxx            # API key
export LOREM_PICSUM_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `lorem-picsum_list` tool
...
### Call the `lorem-picsum_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `lorem-picsum_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `lorem-picsum_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LOREM_PICSUM_APIKEY` | API key sent with every request. |
| `LOREM_PICSUM_BASE` | Optional override of the API base URL. |

### Entities

The 9 entities valid as the `entity` argument:

get_random_image | get_random_square_image | height | heightwebp | id_info | idn | list | seed | seed_info

### Smoke test via HTTP (raw JSON-RPC)

```sh
./lorem-picsum-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"lorem-picsum_load","arguments":{"entity":"get_random_image","query":{"id":1}}}}'
```
